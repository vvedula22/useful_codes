...
!***********************************************************************
     ...
     &   'Number of spatial dimensions (nsd):   '
      ...
      if (nsd.ne.2 .and. nsd.ne.3) then
         write(stdout,ftab4) "ERROR: problem dimension can be 2 or 3"
      end if
      write(stdout,*)

      write(stdout,ftab1,advance='no') &
     &   'Number of edges along x-direction:   '
      read(*,*,iostat=i) nx
      write(stdout,ftab1) "-----------------------------------"
      ...
      end program structured_mesher

!***********************************************************************

      subroutine tri_mesher()
      use varmod
      implicit none

      integer :: a, b, e, g, i, j, k, Ac, iFa, nNo, nEl, nid(4), &
     &   iord(3,2)
      real(kind=8) :: vol, Jac, Ks(2,2)
...
!     Tranform coordinates into mesh structure
      msh%nNo = nx*ny
...
!           Decompose square element into 2 triangles
            do k=1, 2
               e = e + 1
               do a=1, 3
                  b = iord(a,k)
...
!     Compute jacobian
      ...
      write(stdout,ftab1) "Mesh <"//TRIM(msh%name)//"> area: "// &
     &   TRIM(STR(vol))
...
!     Left face (X = 0)
      ...
      do j=1, ny-1
         e = e + 1
         msh%fa(iFa)%IEN(1,e) = j
         msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(1,e) + 1
         msh%fa(iFa)%gE(e)    = (j-1)*2*(nx-1) + 2
      end do
...
!     Bottom face (Y = 0)
      ...
      msh%fa(iFa)%nNo = nNo
      msh%fa(iFa)%nEl = nEl
      write(msh%fa(iFa)%name,'(A)') "bottom"
      call faceAlloc(msh%fa(iFa))
      a = 0
      do i=1, nx
         a  = a + 1
      ...
      do i=1, nx-1
         e = e + 1
         msh%fa(iFa)%IEN(1,e) = i
         msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(1,e) + 1
         msh%fa(iFa)%gE(e)    = 2*i-1
      end do
...
!     Top face (Y = 1)
      ...
      do i=1, nx-1
         ...
         msh%fa(iFa)%IEN(1,e) = i
         msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(1,e) + 1
         msh%fa(iFa)%gE(e)    = 2*((ny-2)*(nx-1) + i)
      end do

      return
      end subroutine tri_mesher

!***********************************************************************

      subroutine tet_mesher()
      use varmod
      implicit none

      integer :: a, b, e, g, i, j, k, l, Ac, iFa, nNo, nEl, nid(8), &
     &   iord(4,6)
      real(kind=8) :: vol, Jac, Ks(3,3)
...
!     Face Z = 1
      ...
      do j=1, ny-1
         ...
         do i=1, nx-1
            ...
            msh%fa(iFa)%IEN(1,e) = (j-1)*nx + i
            msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(1,e) + 1
            msh%fa(iFa)%IEN(3,e) = msh%fa(iFa)%IEN(1,e) + nx + 1
            msh%fa(iFa)%gE(e)    = 6*l + 3

            e = e + 1
            msh%fa(iFa)%IEN(1,e) = msh%fa(iFa)%IEN(1,e-1)
            msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(3,e-1)
...
!     Face X = 1
      ...
      do k=1, nz-1
         ...
         do j=1, ny-1
            ...
            msh%fa(iFa)%IEN(1,e) = (k-1)*ny + j
            msh%fa(iFa)%IEN(2,e) = msh%fa(iFa)%IEN(1,e) + 1
            msh%fa(iFa)%IEN(3,e) = msh%fa(iFa)%IEN(1,e) + ny + 1
            msh%fa(iFa)%gE(e)    = 6*l + 1
