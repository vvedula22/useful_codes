...
%% add paths
srcDir = '/media/vvedula/Data/Projects/Zebrafish/01-case-4D/04-new-cases/00-ImRegister';
addpath(genpath(srcDir));

switch fishType
    case 1
        projDir = sprintf('%s/../%02d-%ddpf/01-control/fish%02d',...
            srcDir,dpf-1,dpf,fishNo);
    case 2
        projDir = sprintf('%s/../%02d-%ddpf/02-smooth_AG/fish%02d',...
            srcDir,dpf-1,dpf,fishNo);
    otherwise
...
nsrf = 3;
for isrf=1:nsrf
    fname = sprintf('%s/03-case-setup/03.1-register_par/%02d_mshsrf_%02d.vertices',projDir,iso,isrf);
    fid = fopen(fname,'r');
    srf(isrf).vertices = fscanf(fid,'%f');
    fclose(fid);
    srf(isrf).vertices = reshape(srf(isrf).vertices, [3, numel(srf(isrf).vertices)/3])';

    fname = sprintf('%s/03-case-setup/03.1-register_par/%02d_mshsrf_%02d.faces',projDir,iso,isrf);
...
for i=0:nref
    ...
    if (i==iso)
        for isrf=1:nsrf
            fname = sprintf('%s/03-case-setup/03.1-register_par/01-morphed-mshsrf_%02d/%02d_registered',projDir,isrf,i);
...
%            stlwrite([fname '.stl'],srf(isrf),'mode','binary');
        ...
        fname = sprintf('%s/03-case-setup/03.1-register_par/00-spline-disps/%02d_splineDisp',projDir,i);
        disp = res.X;
        save([fname '.mat'], 'disp');
        clear disp;
        
        for isrf=1:nsrf
            newsrf.disps = zeros(size(srf(isrf).vertices));
            fprintf('\nSurface morphing..\n');
            newsrf = morphSurf3D(srf(isrf), res, orig, spacing, newim);
            newsrf.disps = newsrf.vertices - srf(isrf).vertices;

            cd(projDir);
            fname = sprintf('%s/03-case-setup/03.1-register_par/01-morphed-mshsrf_%02d/%02d_registered',projDir,isrf,i);
%            fprintf('Writing stl file, mesh surface %d..\n', isrf);
%            stlwrite([fname '.stl'],newsrf,'mode','binary');
            fprintf('Writing vtk file, mesh surface %d..\n', isrf);
            writeVTK(newsrf, [fname '.vtk'], 'a');
        end
    end
end
cd (srcDir);
