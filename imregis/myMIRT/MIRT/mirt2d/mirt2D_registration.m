...
while (abs(fchange)>optim.fundif) && (iter<optim.maxsteps)
        ...
        if ((fp-f)>0),
            ...
            if main.single
                subplot(2,2,3,'align'), imshow(main.refimsmall); title('Ref (fixed)'); drawnow;
                subplot(2,2,4,'align'), imshow(im); title('Reg. Progress..'); drawnow;
                titl = sprintf('%s%d, %s%d','Level=',main.level,'Iter=',iter);
                subplot(2,2,2,'align'), mirt2D_meshplot(X(:,:,1),X(:,:,2));  title(titl); drawnow;
            end
...
if (iter>=optim.maxsteps)
    disp('***********************');
    disp('Exceeded maximum iterations before convergence..');
    disp('Try increasing for better convergence..');
    disp(['Final residual: ' num2str(fchange)]);
    disp('***********************');
end
