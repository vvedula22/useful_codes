...
% subplot(2,2,1); imshow(im); title('Source'); drawnow;
% subplot(2,2,2); imshow(refim); title('Ref (fixed)'); drawnow;
% subplot(2,2,3); histogram(im(:),256); title('Hist (source)'); drawnow;
% subplot(2,2,4); histogram(refim(:),256); title('Hist (ref)'); drawnow;
...
for level=1:main.subdivide
    ...
    imsmall(imsmall<0)=0;
    imsmall(imsmall>1)=1;
    subplot(2,2,1,'align'), imshow(imsmall); title('Source'); drawnow;
...
disp('MIRT: 2D B-spline based registration complete.')
disptime(toc);
