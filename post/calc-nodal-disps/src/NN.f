...
!     Here parameters related to the elemnt are set
     ...
     2      lM%Nx(:,:,g))
      ...
      ALLOCATE(lM%xib(2,insd), lM%Nb(2,lM%eNoN))
...
!     This routine selects boundary element type
     ...
     2   lFa%N(lFa%eNoN,lFa%nG), lFa%Nx(nsd,lFa%eNoN,lFa%nG))
...
!     using Newton's method
     ...
     2   xK(nsd), rK(nsd), Am(nsd,nsd)
      ...
      itr = 0
      xiK = xi
c      WRITE(1000,'(8X,A)') "Newton iterations.."
c      WRITE(1000,'(10X,A)',ADVANCE='NO') "Initial xi: "
c      DO i=1, nsd
c         WRITE(1000,'(A)',ADVANCE='NO') " "//STR(xi(i))
c      END DO
c      WRITE(1000,'(A)')
      DO
         itr = itr + 1
...
c         WRITE(1000,'(10X,A)') "Iter: "//STR(itr)//" "//
...
!     Newton's method converges
         flag = .TRUE.
c         WRITE(1000,'(10X,A)') "Success.."
      ELSE
!     Newton's method failed to converge
         flag = .FALSE.
c         WRITE(1000,'(10X,A)') "Fail.."
      END IF
...
!--------------------------------------------------------------------
!     This routine returns a vector at element "e" face "lFa" that is
!     the normal weigthed by Jac, i.e. Jac = SQRT(NORM(n)).
      SUBROUTINE GNNB(lFa, e, insd, eNoNb, Nx, n)
      USE COMMOD
      USE ALLFUN
      IMPLICIT NONE
