!#######################################################################
      ...
      CONTAINS
!-----------------------------------------------------------------------
      SUBROUTINE TTP_INIT(zid, nX, X, X0)
      IMPLICIT NONE
...
!     Time integration performed using Forward Euler method
      SUBROUTINE TTP_INTEGFE(zid, nX, X, Ts, dt, Istim, RPAR)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: zid, nX
      ...
      CALL TTP_GETF(zid, nX, Ts, X, f, Istim, RPAR)
      X = X + dt*f
...
!     Time integration performed using 4th order Runge-Kutta method
      SUBROUTINE TTP_INTEGRK(zid, nX, X, Ts, dt, Istim, RPAR)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: zid, nX
      REAL(KIND=8), INTENT(INOUT) :: X(nX), RPAR(18)
      REAL(KIND=8), INTENT(IN) :: Ts, dt, Istim
...
!     RK4: 1st pass
      trk = Ts
      Xrk(:,1) = X(:)
      CALL TTP_GETF(zid, nX, trk, Xrk(:,1), frk(:,1), Istim, RPAR)

!     RK4: 2nd pass
      trk = Ts + dt/2.0D0
      Xrk(:,2) = X(:) + dt*frk(:,1)/2.0D0
      CALL TTP_GETF(zid, nX, trk, Xrk(:,2), frk(:,2), Istim, RPAR)

!     RK4: 3rd pass
      trk = Ts + dt/2.0D0
      Xrk(:,3) = X(:) + dt*frk(:,2)/2.0D0
      CALL TTP_GETF(zid, nX, trk, Xrk(:,3), frk(:,3), Istim, RPAR)

!     RK4: 4th pass
      trk = Ts + dt
      Xrk(:,4) = X(:) + dt*frk(:,3)
      CALL TTP_GETF(zid, nX, trk, Xrk(:,4), frk(:,4), Istim, RPAR)
...
!     Time integration performed using Crank-Nicholson method
      SUBROUTINE TTP_INTEGCN2(zid, nX, Xn, Ts, dt, Istim, IPAR, RPAR)
      USE MATFUN
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: zid, nX
     ...
     2   Im(nX,nX), JAC(nX,nX), rmsA, rmsR
      ...
      Im = MAT_ID(nX)
      CALL TTP_GETF(zid, nX, Ts, Xn, fn, Istim, RPAR)
      ...
      DO
         k = k + 1
         CALL TTP_GETF(zid, nX, t, Xk, fk, Istim, RPAR)
...
!-----------------------------------------------------------------------
      SUBROUTINE TTP_GETF(i, n, time, X, dX, I_stim, RPAR)
      IMPLICIT NONE
...
!-----------------------------------------------------------------------
      SUBROUTINE TTP_GETJ(i, n, time, X, JAC)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: i, n
...
!-----------------------------------------------------------------------
      SUBROUTINE TTP_ACTCPL(nX, X, dt, eps_Ca, Tact)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: nX
      REAL(KIND=8), INTENT(IN) :: X(nX), dt
      REAL(KIND=8), INTENT(OUT) :: eps_Ca
      ...
      eps_Ca = eps_0 + (eps_i - eps_0)*EXP(-EXP(-xi_T*(X(4) - Ca_crit)))
      nr     = Tact + (eps_Ca*dt* eta_T*(X(4)-Ca_rest))
      Tact   = nr / (1.0D0 + eps_Ca*dt)

      RETURN
      END SUBROUTINE TTP_ACTCPL
!-----------------------------------------------------------------------
      END MODULE TTPMOD
!#######################################################################
